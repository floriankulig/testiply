  ...
  const getInitialApps = async (): Promise<GameRowApps> => {
    ...
    try {
      const res: AxiosResponse<{ apps: GameRowApps }> = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/getGames?platform=${selectedPlatform.id}`
      );
      setLoading(false);
  ...
  const getCategoryApps = async (): Promise<App[]> => {
    ...
    try {
      const res: AxiosResponse<{ apps: CategoryApps }> = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/getGames?platform=${selectedPlatform.id}&sort=${selectedCategory.id}`
      );
      setLoading(false);
